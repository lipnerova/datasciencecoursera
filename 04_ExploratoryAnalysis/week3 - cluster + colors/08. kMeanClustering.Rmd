...
text (cx+0.05, cy+0.05, label=c(rep('pos1',3)))
...
points(x,y,pch=19,cex=2,col=cols1[newClust]) # paint points

## recalculate centroids
newCx <- tapply(x,newClust,mean)
newCy <- tapply(y,newClust,mean)
points(newCx,newCy,col=cols1,pch=3,cex=2,lwd=2)
text (newCx+0.05, newCy+0.05, label=c(rep('pos2',3)))

## Iteration 2 - Find the closest centroid
distTmp <- matrix(NA,nrow=3,ncol=12)
...
points(x,y,pch=19,cex=2,col=cols1[newClust2]) # paint points

## Iteration 2 - recalculate centroids
newCx <- tapply(x,newClust2,mean)
newCy <- tapply(y,newClust2,mean)
points(newCx,newCy,col=cols1,pch=3,cex=2,lwd=2)
text (newCx+0.05, newCy+0.05, label=c(rep('pos3',3)))

## Final iteration - Find the closest centroid
distTmp <- matrix(NA,nrow=3,ncol=12)
distTmp[1,] <- (x-newCx[1])^2 + (y-newCy[1])^2
distTmp[2,] <- (x-newCx[2])^2 + (y-newCy[2])^2
distTmp[3,] <- (x-newCx[3])^2 + (y-newCy[3])^2
finalClust <- apply(distTmp,2,which.min)
points(x,y,pch=19,cex=2,col=cols1[finalClust]) # paint points

## Final centroids
finalCx <- tapply(x,finalClust,mean)
finalCy <- tapply(y,finalClust,mean)
points(finalCx,finalCy,col=cols1,pch=3,cex=2,lwd=2)
text (finalCx+0.05, finalCy+0.05, label=c(rep('pos4',3)))
