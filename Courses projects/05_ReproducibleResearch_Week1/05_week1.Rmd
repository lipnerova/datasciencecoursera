...
## 1. Loading & prepping the Dataset
...
+ Intervals show the time of day with format **HH:mm**: we jump from "55" to "100", from "155" to "200", etc.
...
head (levels(factor(activity$interval)), 25)
...
```{r, echo=FALSE}
# clean up variables
rm (dailyActivity, dailyMean, dailyMedian, g)
```
...
```{r, echo=FALSE}
# clean up variables
rm (mostActiveInterval, mostNumberOfSteps, g)
```
...
```{r, echo=FALSE}
# clean up variables
rm (intervalActivity, fullActivityMelt, fullDailyActivity, g, mean1, mean2, median1, median2, myMeanMedian)
```

\  

## 5. Average daily activity pattern - weekdays vs weekends
...
_Note: we will use the complete dataset, where the NA values have been replaced by the interval average._
...
```{r, echo=FALSE}
# clean up variables
#rm (fullActivity,intervalActivity, intervalActivityAll, g)
```
